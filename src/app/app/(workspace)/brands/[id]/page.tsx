...
export default async function BrandHomePage({
  ...
  const competitorsTab = tab === "competitors";
  const citationsTab = tab === "citations";
  const overviewTab = !competitorsTab && !citationsTab;
  ...
  let citationGroups: Array<{ url: string; count: number }> = [];
  if (citationsTab && latestRun) {
    const cited = await ctx.db
      .select({ citedUrls: runRows.citedUrls })
      .from(runRows)
      .where(eq(runRows.runId, latestRun.id));
    ...
    citationGroups = [...tally.entries()]
      .map(([url, count]) => ({ url, count }))
      .sort((a, b) => b.count - a.count || a.url.localeCompare(b.url));
  }

  const tabs = [
    { href: `/app/brands/${brand.id}`, label: "Overview", active: overviewTab },
    { href: `/app/brands/${brand.id}/prompts`, label: "Questions", active: false },
    { href: `/app/brands/${brand.id}/history`, label: "Briefs", active: false },
    { href: `/app/brands/${brand.id}?tab=competitors`, label: "Competitors", active: competitorsTab },
    { href: `/app/brands/${brand.id}?tab=citations`, label: "Citations", active: citationsTab },
    { href: `/app/brands/${brand.id}/edit`, label: "Settings", active: false },
  ];
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap gap-2">
          {latestReport ? (
            <Button asChild variant="outline">
              <a href={`/api/reports/${latestReport.id}/download`}>Download PDF</a>
            </Button>
          ) : null}
          <RunNowButton
            brandId={brand.id}
            promptId={recheckPrompt?.id}
            label={recheckPrompt ? "Recheck" : "Run brief"}
            disabled={
              Boolean(brand.archivedAt) ||
              prompts.length === 0 ||
              isSampleBrand(brand.kind) ||
              (isPitchBrand(brand.kind) && Boolean(latestRun))
            }
            hint={
              isSampleBrand(brand.kind)
                ...
                : isPitchBrand(brand.kind) && latestRun
                  ...
                  : brand.archivedAt
                    ? "Restore this client before you run."
                    : prompts.length === 0
                      ? `Generate ${promptCap} buyer questions before you run.`
                      : recheckPrompt
                        ? "Runs all buyer questions for this client; focuses the selected prompt after queueing."
                        : null
            }
          />
        </div>
      ...
      <nav className="mb-8 flex flex-wrap gap-2" aria-label="Client">
        {tabs.map((item) => (
          <Link
            key={item.label}
            href={item.href}
      ...
      {competitorsTab ? (
        <div className="rounded-cb-card border border-cb-line bg-cb-surface p-5">
          ...
          {insights.competitorLeader ? (
            ...
            <p className="mt-2 text-sm text-cb-muted">Leader stats appear after the first brief.</p>
          )}
        ...
        <div className="rounded-cb-card border border-cb-line bg-cb-surface p-5">
          ...
          <p className="mt-2 text-sm text-cb-muted">Cited URLs from the latest run, grouped by URL.</p>
          {citationGroups.length === 0 ? (
            <p className="mt-4 text-sm text-cb-muted">No cited URLs yet. Run a brief to fill this list.</p>
          ) : (
            <ul className="mt-4 space-y-2 text-sm">
              {citationGroups.map((row) => (
                <li key={row.url} className="flex items-start justify-between gap-4 border-b border-cb-line py-2 last:border-0">
                  <a href={row.url} target="_blank" rel="noreferrer" className="min-w-0 break-all text-cb-accent">
                    {row.url}
                  </a>
                  <span className="shrink-0 font-mono tabular-nums text-cb-muted">{row.count}</span>
                </li>
              ))}
            </ul>
          )}
        ...
        <>
          {recheckPrompt ? (
            <div className="mb-8 rounded-cb-card border border-cb-accent bg-cb-surface p-5">
              ...
              <p className="mt-1 text-sm text-cb-muted">
                Mix: {recheckPrompt.mix || "—"}. Queuing a run refreshes every prompt for this client and opens the run
                with this question highlighted.
              </p>
          ...
          <div className="mb-8 grid gap-4 md:grid-cols-3">
            <div className="rounded-cb-card border border-cb-line bg-cb-surface p-5">
              ...
              <p className="mt-2 text-sm text-cb-muted">
                {score == null
                  ? "No brief for this period yet."
                  : `Named in ${score} of ${total} unbranded buyer questions.`}
              </p>
              <div className="mt-2">
            ...
            <div className="rounded-cb-card border border-cb-line bg-cb-surface p-5">
              ...
              <p className="mt-2 text-sm text-cb-muted">
                {recommended == null
                  ? "Recommendation score appears after the first brief."
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-sm">
                {latestReport ? (
                  <>
                    ...
                    <a href={`/api/reports/${latestReport.id}/download`} className="text-cb-accent">
                      Download PDF
                    </a>
                  </>
                ) : prompts.length === 0 ? (
                  <Link href={`/app/brands/${brand.id}/prompts`} className="text-cb-accent">
                    Generate {promptCap} questions
                  </Link>
                ) : null}
                {latestRun ? (
            ...
            <div className="rounded-cb-card border border-cb-line bg-cb-surface p-5">
              ...
              {insights.missingQuestions.length === 0 ? (
                <p className="mt-3 text-sm text-cb-muted">
                  {latestReport
                    ? "No missing questions in the latest brief."
                    : "Missing questions appear after the first brief."}
                </p>
              ) : (
          ...
          <div className="mb-8 rounded-cb-card border border-cb-line bg-cb-surface p-5">
            ...
            <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
              ...
              <Item label="Questions" value={`${prompts.length} / ${promptCap}`} />
